...
export function render(container, ctx) {
  ...
  const emoji = el('div', { class: 'tt-emoji' });
  ...
  const root = el('div', { class: 'screen timer k-getready' },
    ...
    el('div', { class: 'tt-stage' },
      stateBadge, emoji, name, desc,
      count,
      el('div', { class: 'bar seg' }, segBar),
      nextLine),
    el('div', { class: 'tt-bottom' },
      el('div', { class: 'bar total-wrap' }, totalBar),
      elapsed,
      el('div', { class: 'ctrl-row' },
        el('button', { class: 'btn round', 'aria-label': t('prev'), on: { click: () => engine.prev() } }, '⏮'),
        pauseBtn,
        el('button', { class: 'btn round', 'aria-label': t('next'), on: { click: () => engine.next() } }, '⏭'),
  ...
  function applyState(segment) {
    ...
    stateBadge.textContent = t(labelKeyForKind(segment.kind));
    ...
    topPhase.textContent = segment.round ? t('roundXofY', segment.round, segment.totalRounds) : t(labelKeyForKind(segment.kind));
  }
  ...
  function applyTick(s) {
    ...
    nextLine.textContent = s.next ? `${t('nextUp')}: ${s.next.emoji || ''} ${tx(s.next.name)}` : '';
  ...
  // 시작 직후 들어오는 이벤트로 채워지지만, 가능한 경우 즉시 1회 페인트
