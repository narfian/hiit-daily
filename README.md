...
1. 생성 프롬프트 문서를 만듭니다(이미 포함되어 있음): `node tools/gen-prompts.mjs` → **`images/PROMPTS.md`**(운동) · **`images/PROMPTS-warmup.md`**(워밍업) · **`images/PROMPTS-cooldown.md`**(쿨다운). 워밍업/쿨다운 이미지도 같은 `images/exercises/` 폴더에 `warmup-*.png` / `cooldown-*.png` 로 넣으면 이모지/SVG를 대체합니다.
